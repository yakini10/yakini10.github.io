...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (!empty($_COOKIE['login']) && empty($_SESSION['user_id'])) {
        $messages[] = "<div class='success'>
            ...
            <strong>Логин :</strong> " . e($_COOKIE['login']) . "<br>
            <strong>Пароль :</strong> " . e($_COOKIE['password']) . "<br>
    ...
    foreach ($error_fields as $field) {
        if (!empty($_COOKIE[$field . '_error'])) {
            $errors[$field] = true;
            $messages[] = '<div class="error">' . e($_COOKIE[$field . '_error_msg']) . '</div>';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['auth'])) {
    ...
    if (!isset($_POST['csrf_token']) || !verifyCSRFToken($_POST['csrf_token'])) {
        $messages[] = "<div class='error'>Ошибка безопасности: форма устарела</div>";
        include('form.php');
        exit();
    }
    ...
    // AJOUTÉ - Validation des langages (SQL Injection)
    $allowed_langs = array_keys(getLanguagesList());
    if (empty($_POST['languages'])) {
        ...
        foreach ($_POST['languages'] as $lang) {
            if (!in_array($lang, $allowed_langs)) {
                $has_errors = true;
                setError('languages', 'Обнаружен недопустимый язык');
                break;
            }
        }
    }
